...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await login(email, password, useOtp);
    } catch {
    } finally {
      setLoading(false);
    }
          ...
          >
            <div className="glass-strong rounded-3xl p-8 md:p-10 animate-scale-in">
              ...
              <div className="text-center mb-8">
                ...
                >
                  Your AI Shopping Universe Awaits
                </motion.p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                >
                  <label className="flex items-center gap-2 text-[--muted] cursor-pointer group">
                    <input
                      type="checkbox"
                      checked={useOtp}
                      onChange={(e) => setUseOtp(e.target.checked)}
                      className="rounded border-white/20 bg-transparent accent-[--secondary]"
                    />
                    <span className="group-hover:text-white transition-colors">Use OTP verification</span>
                  </label>
                  <Link href="/forgot-password" className="text-[--secondary] hover:text-white transition-colors">
                    Forgot password?
                  </Link>
                </motion.div>
                ...
                {useOtp && (
                  <motion.p
                    ...
                    className="text-xs text-[--muted] text-center font-sans"
                  >
                    A verification code will be sent to your email after password validation.
                  </motion.p>
                )}
